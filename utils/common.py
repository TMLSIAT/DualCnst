...
def get_test_labels(args, loader = None):
    if args.in_dataset == "ImageNet":
        ...
        test_labels = obtain_ImageNet_classes()
    elif args.in_dataset == "ImageNet_and_R":
        test_labels = obtain_UCM_classes()
    elif args.in_dataset == "ImageNet_Robust":
        test_labels = obtain_ImageNet_classes()
    elif args.in_dataset == "ImageNet10":
        test_labels = obtain_ImageNet10_classes()
    elif args.in_dataset == "TestTrain":
        test_labels = obtain_ImageNet10_classes()
    elif args.in_dataset == "ImageNet20":
        test_labels = obtain_ImageNet20_classes()
    elif args.in_dataset == "ImageNet100":
        test_labels = obtain_ImageNet100_classes()
    elif args.in_dataset == "ImageNet_and_ARSV":
        test_labels = obtain_ImageNet_classes()
...
def obtain_ImageNet10_classes():
    class_dict = {
        ...
        'brambling bird': 'n01530575',
        "Siamese cat": 'n02123597',
        'antelope': 'n02422699',
        'swiss mountain dog': 'n02107574',
        "bull frog": "n01641577",
...
def obtain_ImageNet_and_R_classes():
    from OODNegMining.classs_name import imagenet_classes, imagenet_r
    formatted_imagenet_r = [cls.replace('_', ' ') for cls in imagenet_r]
    combined_classes = list(set(imagenet_classes + formatted_imagenet_r))
    return combined_classes

def obtain_UCM_classes():
    from OODNegMining.classs_name import ucm_classes
    return ucm_classes

def obtain_ImageNet_and_A_classes():
    from OODNegMining.classs_name import imagenet_classes, imagenet_a
    combined_classes = list(set(imagenet_classes + imagenet_a))
    return combined_classes

def obtain_ImageNet_and_S_classes():
    from OODNegMining.classs_name import imagenet_classes, imagenet_s
    try:
        combined_classes = list(set(imagenet_classes + imagenet_s))
    except:
        combined_classes = imagenet_classes
    return combined_classes

def obtain_ImageNet_and_V2_classes():
    from OODNegMining.classs_name import imagenet_classes, imagenet_v2
    try:
        combined_classes = list(set(imagenet_classes + imagenet_v2))
    except:
        combined_classes = imagenet_classes
    return combined_classes
...
def get_num_cls(args):
    NUM_CLS_DICT = {
        ...
        'bird200': 200, 
        'ImageNet': 1000,
        'ImageNet_and_ARSV': 1000,
        'ImageNet_and_R': 1000,
        'ImageNet_Robust': 1000,
        'UCM': 21,
        'cifar10': 10,
        'cifar100': 100,
        'CXR': 12,
...
def accuracy(output, target, topk=(1,)):
    """Compute precision@k for specified k values."""
    maxk = max(topk)
