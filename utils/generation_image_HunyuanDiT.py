...
from modelscope import HunyuanDiTPipeline
import torch
...
pipe = HunyuanDiTPipeline.from_pretrained("/data0/fayi/HunyuanDiT", torch_dtype=torch.float16)
...
if __name__ == '__main__':
    positive_samples_path = PROJECT_ROOT / 'OODNegMining' / 'ImageNet' / 'positive' / 'positive_samples.txt'
    low_similarity_neg_samples_path = PROJECT_ROOT / 'OODNegMining' / 'ImageNet' / 'negative' / 'c.txt'
    ...
    generation(combined_samples, img_dir='/data0/fayi/generation_image_DiT', num_images=1, batch_size=8)